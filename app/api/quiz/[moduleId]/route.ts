...
export async function GET(_req: Request, { params }: { params: { moduleId: string } }) {
  ...
  if (!qz) {
    ...
    qz = await prisma.quiz.create({ data: { moduleId, passScore: 70, timeLimitSeconds: 300 } });
  }
  const questions = qz.questions?.length ? qz.questions : await prisma.question.findMany({ where: { quizId: qz.id, active: true } });
